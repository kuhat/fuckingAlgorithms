 ...
 */
    ...
    class HitCounter {
        ...
        public int getHits(int timestamp) {
            while(!qu.isEmpty() && timestamp - qu.peek() >= 300) {
                qu.poll();
            }
            return qu.size();
        }
     ...
     * void put(int key, int value) inserts a (key, value) pair into the HashMap. If the key already exists in
     * the map, update the corresponding value.
    ...
    class MyHashMap {
        ...
        public void put(int key, int value) {
            ...
            if (lists[pos] == null) lists[pos] = new LinkedList<Node>();
            LinkedList<Node> tmp = lists[pos];
            for (Node item : tmp) {
                if (item.key == key) {
                    tmp.remove(item);
                    break;
        ...
        public int get(int key) {
            int pos = key % len;
            LinkedList<Node> bucket = lists[pos];
    ...
    class ProductOfNumbers {
        ...
        public void add(int num) {
            if (num == 0) {
                ...
                if (prod.size() == 0) prod.add(num);
                else prod.add(prod.get(prod.size() - 1) * num);
            }
        }

        public int getProduct(int k) {
            if (prod.size() < k) return 0;
            else if (prod.size() == k) return prod.get(prod.size() - 1);
